...
export const Route = createFileRoute("/app/agent-services/")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Turn on persistent workers, like the Telegram and trading monitors." },
    ],
  }),
...
function AgentServices() {
  ...
  const telegramLabel = !telegramConfigured ? "Not configured" : telegramStatus === "active" ? "Active" : "Paused";
  const telegramTone = telegramLabel === "Active" ? "text-mint" : telegramLabel === "Paused" ? "text-amber" : "text-mute";
  const telegramDot = telegramLabel === "Active" ? "bg-mint" : telegramLabel === "Paused" ? "bg-amber" : "bg-mute";

  const tradingStatus = trading.data?.status;
  const tradingConfigured = !!(trading.data?.config?.pair || trading.data?.config?.timeframe || trading.data?.config?.connector);
  const tradingLabel = !tradingConfigured ? "Not configured" : tradingStatus === "active" ? "Active" : "Paused";
  const tradingTone = tradingLabel === "Active" ? "text-mint" : tradingLabel === "Paused" ? "text-amber" : "text-mute";
  const tradingDot = tradingLabel === "Active" ? "bg-mint" : tradingLabel === "Paused" ? "bg-amber" : "bg-mute";
  ...
  return (
    <>
      ...
      <div className="space-y-6 p-4 lg:p-8">
        ...
        {(telegram.isLoading || trading.isLoading) && <p className="text-sm text-mute">Loading services…</p>}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Panel>
            <div className="flex items-center gap-3">
              ...
              <span className={`ml-auto inline-flex items-center gap-1.5 font-mono text-[11px] ${telegramTone}`}>
            ...
            <p className="mt-3 text-sm text-fog">
              Watches Telegram chats you choose and normalizes the messages into structured trading signals.
            </p>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 border-t border-line pt-4 font-mono text-[11px]">
              <div>
                ...
                <dd className="mt-1 text-fog">{telegram.data?.config?.monitored_chats?.length ?? 0}</dd>
              </div>
              <div>
                <dt className="text-mute">Alerts to</dt>
                <dd className="mt-1 truncate text-fog">{telegram.data?.config?.alert_chat ?? "me"}</dd>
              </div>
            </dl>
          ...
          <Panel>
            <div className="flex items-center gap-3">
              ...
              <span className={`ml-auto inline-flex items-center gap-1.5 font-mono text-[11px] ${tradingTone}`}>
